# This file was taken from the publication "Approximating CNNs with Bag-of-local-Features models works surprisingly well on ImageNet" by Brendel & Bethge (2019).
# It was slightly adapted to enable regularization (see cc_training.py).
...
class BagNet(nn.Module):
    ...
    def __init__(self, regularization, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_classes=1000, avg_pool=True):
        self.inplanes = 64
    ...
    def forward(self, x, regularization):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.bn1(x)
        x = self.relu(x)
...
def bagnet32(regularization = 0, pretrained=True, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-32 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(regularization, Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[1,1,1,1], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['bagnet32']))
    return model

def bagnet16(regularization = 0, pretrained=True, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-16 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(regularization, Bottleneck, [ 3, 4, 6, 3], strides=strides, kernel3=[1,1,1,0], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['bagnet16']))
    return model

def bagnet8(regularization = 0, pretrained=True, strides=[2, 2, 2, 1], **kwargs):
    """Constructs a Bagnet-8 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = BagNet(regularization, Bottleneck, [3, 4, 6, 3], strides=strides, kernel3=[1,1,0,0], **kwargs)
    if pretrained:
